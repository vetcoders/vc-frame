...
* fix(compact-bar): wheel events over the one-row chrome target the exact focused terminal pane at its cursor (with a content-center fallback) instead of switching tabs, preserving native mouse-aware TUI scrolling, alternate-screen fallback and ordinary scrollback without coordinate hit-test races; the new plugin command requires stdin permission and rejects requests above 100 lines
