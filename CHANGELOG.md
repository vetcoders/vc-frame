...
* fix(logging): creating a session socket dir chmod's `/tmp/vc-frame-<uid>` to `0700`, not only the nested contract directory — `create_dir_all` no longer leaves the uid tmp root at umask `0755`
* fix(logging): the active `vc-frame.log` is chmod `0600` after log4rs opens it (log4rs `create(true)` otherwise inherits umask `0644`)
