...
* fix(server): the plugin host resolves the reserved `vc-frame:self` token for background `run_command` executions too, not only for pane-command paths — plugin-driven guest creates and project handoffs used to exec the literal token and die ENOENT before any CLI ran
* feat(operator-frame): wave 1 of the host/guest Operator Frame — the session rail gains a pinned `Operator Frame` host section (Dashboard · Active runs · Config · Doctor · Projects) that never scrolls off and keeps feed truth distinct (unknown `?`, confirmed count, stale `!`); the compact bar projects the visited guest's organs (Overview / Agents / Shell) with a fixed `Voc` host-console chip in the protected right zone (48 cols, narrow bars shed chips theme→Voc instead of overflowing); floating panes become a scoped Panels layer — Global (pinned) survives every guest visit, Project panels hide and restore with their guest — with a wrapping `i/N` pager and `PanelsNext` / `PanelsPrevious` / `PanelsSetScope` actions through the plugin API and the client↔server wire contract (version 3); the bottom bar's center projects the visited guest (`workspace · repo · task`) from the guest-surface push plus the canonical `vc.live-runs.v1` feed, shedding feed-derived fields when the feed degrades and clearing on an explicit guest-death tombstone
...
* fix(logging): a server-side reaper deletes `client-<pid>/` dirs that carry `.vc-frame-cli-client` or are empty pre-lazy leftovers, whose process is gone and whose mtime is ≥ 60 s (cap 2000 per pass, every 15 minutes after start, one flock per uid) so leftover piles do not grow forever and a session named `client-123` is not swept
...
* fix(logging): web-server IPC mkdir, `--server` bind/rebind, and `--debug` pane capture go through the same `0700` tmp-root helper as CLI clients
* fix(logging): socket mkdir tightens the uid tmp root only when the socket dir lives under it — XDG / `VC_FRAME_SOCKET_DIR` no longer mkdir `/tmp/vc-frame-<uid>` as a side effect
...
* feat(clinic): `asset-integrity` doctor section — the binary hashes its embedded plugins at runtime against its own SHA-256 build receipt, so a build that mixes plugin generations says so instead of "jakoś działa" (CRITICAL → install the unified artifact)
