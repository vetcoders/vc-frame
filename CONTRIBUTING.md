...
vc-frame uses the excellent [`log`](https://crates.io/crates/log) crate to handle its internal logging. Each server owns `/$temp_dir/vc-frame-<UID>/vc-frame-log/<session-socket>/vc-frame.log`; short-lived clients use a `client-<PID>` scope. Server-scoped files prevent independent sessions from rotating one shared inode and losing live diagnostics. Those directories and the log file are created on the **first log write**, not at process start, so `--help`, `--version`, `list-sessions` and `action` leave no empty `client-<pid>` residue. The process tmp root `/tmp/vc-frame-<uid>` is chmod `0700` both when the first record lands and when a client creates a session socket directory **under that root** (macOS default; an XDG or `VC_FRAME_SOCKET_DIR` override does not mkdir `/tmp`). The active `vc-frame.log` is chmod `0600` after log4rs opens it. On macOS `$temp_dir` is `/tmp`; elsewhere it follows [std::env::temp_dir()](https://doc.rust-lang.org/std/env/fn.temp_dir.html).
